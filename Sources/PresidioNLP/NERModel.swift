...
// matrix unconstrained. See the README for the measured cost, and GEMM.swift for
// the opt-in Accelerate path.

import Foundation

// Pure-Swift spaCy v3 NER inference.
// Stdlib only: no NaturalLanguage, no C deps. The one optional exception is
// Accelerate, behind the `PresidioAccelerate` trait -- see GEMM.swift.
#if canImport(Darwin)
import Darwin
