...
func readFile(_ path: String) -> [UInt8] {
    let f = fopen(path, "rb")!
    fseek(f, 0, SEEK_END); let n = ftell(f); fseek(f, 0, SEEK_SET)
    var buf = [UInt8](repeating: 0, count: n)
    buf.withUnsafeMutableBytes { _ = fread($0.baseAddress, 1, n, f) }
    fclose(f); return buf
}
...
// ============================ main ============================
func now() -> Double { var ts = timespec(); clock_gettime(CLOCK_MONOTONIC, &ts); return Double(ts.tv_sec) + Double(ts.tv_nsec) * 1e-9 }

let args = CommandLine.arguments
let modelDir = args.count > 1 ? args[1] : "sm/en_core_web_sm/en_core_web_sm-3.7.1"
let inputPath = args.count > 2 ? args[2] : ""
let mode = args.count > 3 ? args[3] : "check"

let t0 = now()
let model = NERModel(dir: modelDir)
print("load model: \(Int((now()-t0)*1000)) ms, \(model.actMove.count) actions")

func FileHandleErr(_ s: String) { fputs(s + "\n", stderr) }

if inputPath.isEmpty {
    let toks = ["John", "Smith", "called", "Acme", "Corp", "in", "Berlin", "on", "March", "3rd", "2021", "."]
    for e in runNER(model, toks) { print("\(e.start)\t\(e.end)\t\(e.label)\t\(toks[e.start..<e.end].joined(separator: " "))") }
} else {
    // input: one sentence per line, tokens separated by \t
    let raw = String(decoding: readFile(inputPath), as: UTF8.self)
    ...
    var lines: [[String]] = []; var normLines: [[String]] = []
    for l in rawLines {
        var ts: [String] = []; var ns: [String] = []
        for f in l {
            if let r = f.firstIndex(of: "\u{1f}") { ts.append(String(f[f.startIndex..<r])); ns.append(String(f[f.index(after: r)...])) }
            else { ts.append(f); ns.append(f.lowercased()) }
        }
        lines.append(ts); normLines.append(ns)
    }
    if mode == "tv" {
        let li = Int(args[4])!
        let ents = runNER(model, lines[li], normLines[li], dump: true)
        _ = ents
    } else if mode == "feat" {
        for (li, l) in lines.enumerated() {
            for (ti, tok) in l.enumerated() {
                print("\(li)\t\(ti)\t\(normLines[li][ti])\t\(lexPrefix(tok))\t\(lexSuffix(tok))\t\(wordShape(tok))")
            }
        }
    } else if mode == "bench" {
        var totalTok = 0
        for l in lines { totalTok += l.count }
        // warm
        for (i,l) in lines.enumerated() { _ = runNER(model, l, normLines[i]) }
        let s = now()
        var reps = 0
        var checksum = 0
        while now() - s < 3.0 { for (i,l) in lines.enumerated() { checksum += runNER(model, l, normLines[i]).count }; reps += 1 }
        let dt = now() - s
        FileHandleErr("tokens=\(totalTok) reps=\(reps) time=\(Double(Int(dt*1000))/1000)s  -> \(Int(Double(totalTok * reps) / dt)) tokens/sec  (checksum \(checksum))")
    } else {
        for (li, l) in lines.enumerated() {
            let es = runNER(model, l, normLines[li])
            print("\(li)\t" + es.map { "\($0.start),\($0.end),\($0.label)" }.joined(separator: " "))
        }
    }
}
