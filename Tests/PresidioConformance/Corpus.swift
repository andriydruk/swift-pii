...
public enum Corpus {
    ...
    /// Adversarial validator inputs and the tri-state Python returned.
    ///
    /// The harvested validator corpus is upstream's own tables, which are
    /// ASCII and well-formed. This one is deliberately hostile — non-ASCII
    /// digits, mixed case, Unicode spaces, combining marks — because that is
    /// where a port drifts from its original.
    public struct ValidatorStressCorpus: Codable, Sendable {
        public struct Case: Codable, Sendable {
            public let input: String
    ...
    public static func validatorStress() throws -> ValidatorStressCorpus {
        try load("validator_stress", as: ValidatorStressCorpus.self)
    }
